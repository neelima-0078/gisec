...
What this does
Trains a small CNN on a clean 10,000-image CIFAR-10 subset as a baseline.
Injects two poisoning attacks at a 5% poison rate:
...
Confirms both attacks are stealthy (poisoned models score within ~2 points of clean baseline on standard test accuracy) while actually working (89.5% attack success rate when the trigger fires).
Runs two independent detectors:
Loss-based outlier detection (catches label-flip poisoning).
Activation clustering (catches backdoor poisoning).
Quarantines flagged samples with a recorded reason for each, retrains from scratch on what's left, and shows the attack collapses to near random-chance (7.4% ASR, down from 89.5%).
Stress-tests the whole cycle across the brief's specified 1-10% poison-rate range, including an honest look at where detection breaks down (~10% poison rate).
Results summary
Metric	Value
...
Open gisec_poisoning_pipeline.ipynb in this repo.
Click "Open in Colab" (or upload the file to https://colab.research.google.com manually).
Runtime → Change runtime type → GPU.
Runtime → Run all. Takes roughly 6-10 minutes for the core pipeline; the poison-rate stress test cell alone takes an additional 15-25 minutes since it trains 10 models.
No manual dataset download needed — CIFAR-10 is fetched automatically on first run.
